...
def order_of_sum(sum_variant):
    if sum_variant.upper() == "A":
        attributes = collect_arithmetic_attributes()
    elif sum_variant.upper() == "G":
        attributes = collect_geometric_attributes()
    return attributes
...
def main():
    ...
    first_sum_attributes = order_of_sum(first_sum_variant)
    
    second_sum_variant = choose_type_of_sum("Är den andra summan aritmetisk [a] eller geometrisk [g]: ")
    second_sum_attributes = order_of_sum(second_sum_variant)
